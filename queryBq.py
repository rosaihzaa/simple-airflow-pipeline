...
def queryBigquery():
    query = '''
        CREATE OR REPLACE TABLE `yourprojectid-datasetid-tableid` AS
        ...
        SELECT DISTINCT
            ...
            Link,
            Address,
            COALESCE(SAFE_CAST(CASE WHEN LOWER(Price) IN ('price on demand', 'contact for price', 'n/a') THEN NULL ELSE REGEXP_REPLACE(Price, r'[\$,]', '') END AS FLOAT64), (SELECT APPROX_QUANTILES(SAFE_CAST(REGEXP_REPLACE(Price, r'[\$,]', '') AS FLOAT64), 100)[OFFSET(50)] FROM `yourprojectid-datasetid-tableid` WHERE SAFE_CAST(REGEXP_REPLACE(Price, r'[\$,]', '') AS FLOAT64) IS NOT NULL LIMIT 1)) AS Price,
            COALESCE(CAST(`Rooms` AS INT64), (SELECT APPROX_QUANTILES(CAST(`Rooms` AS INT64), 100)[OFFSET(50)] FROM `yourprojectid-datasetid-tableid` LIMIT 1))  AS Rooms,
            COALESCE(CAST(`Bedrooms` AS INT64), (SELECT APPROX_QUANTILES(CAST(`Bedrooms` AS INT64), 100)[OFFSET(50)] FROM `yourprojectid-datasetid-tableid` LIMIT 1))  AS Bedrooms,
            ...
            COALESCE(CAST(`Floors` AS INT64), (SELECT APPROX_QUANTILES(CAST(`Floors` AS INT64), 100)[OFFSET(50)] FROM `yourprojectid-datasetid-tableid` LIMIT 1))  AS Floors,
            COALESCE(CAST(REGEXP_EXTRACT(`Living area`, r'\((\d+)\s*m²\)') AS FLOAT64), (SELECT APPROX_QUANTILES(CAST(REGEXP_EXTRACT(`Living area`, r'\((\d+)\s*m²\)') AS FLOAT64), 100)[OFFSET(50)] FROM `yourprojectid-datasetid-tableid` LIMIT 1)) AS Living_area_m2,
            COALESCE(CAST(REGEXP_EXTRACT(`Land`, r'\((\d+)\s*m²\)') AS FLOAT64), (SELECT APPROX_QUANTILES(CAST(REGEXP_EXTRACT(`Land`, r'\((\d+)\s*m²\)') AS FLOAT64), 100)[OFFSET(50)] FROM `yourprojectid-datasetid-tableid` LIMIT 1)) AS Land_m2,
            COALESCE(CAST(REGEXP_EXTRACT(`Total`, r'\((\d+)\s*m²\)') AS FLOAT64), (SELECT APPROX_QUANTILES(CAST(REGEXP_EXTRACT(`Total`, r'\((\d+)\s*m²\)') AS FLOAT64), 100)[OFFSET(50)] FROM `yourprojectid-datasetid-tableid` LIMIT 1)) AS Total_m2,
            COALESCE(CAST(REGEXP_EXTRACT(`Garden`, r'\((\d+)\s*m²\)') AS FLOAT64), (SELECT APPROX_QUANTILES(CAST(REGEXP_EXTRACT(`Garden`, r'\((\d+)\s*m²\)') AS FLOAT64), 100)[OFFSET(50)] FROM `yourprojectid-datasetid-tableid` LIMIT 1)) AS Garden_m2,
            ...
            COALESCE(CAST(`Construction year` AS INT64), (SELECT SAFE_CAST(APPROX_QUANTILES(CAST(`Construction year` AS FLOAT64), 100)[OFFSET(50)] AS INT64) FROM `yourprojectid-datasetid-tableid` LIMIT 1))  AS Construction_year,
            COALESCE(CAST(`Renovation year` AS INT64), (SELECT SAFE_CAST(APPROX_QUANTILES(CAST(`Renovation year` AS FLOAT64), 100)[OFFSET(50)] AS INT64) FROM `yourprojectid-datasetid-tableid` LIMIT 1))  AS Renovation_year,
            COALESCE(SAFE_CAST(ARRAY_LENGTH(SPLIT(REPLACE(REPLACE(Aminities, '[', ''), ']', ''))) AS INT64), (SELECT APPROX_QUANTILES(CAST(ARRAY_LENGTH(SPLIT(REPLACE(REPLACE(Aminities, '[', ''), ']', ''))) AS INT64), 100)[OFFSET(50)] FROM `yourprojectid-datasetid-tableid` WHERE Aminities IS NOT NULL LIMIT 1)) AS Amenities_count,
